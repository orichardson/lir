...
\usepackage{pdgtools}
   ...
   %  \setlength{\marginparwidth}{3.3cm}
    \newcommand\commentout[1]{} % remove material with proper scoping and nesting
...
% The brain continuously reconciles sensory inputs, prior beliefs, and predictions---an operation thought to underlie learning, perception, and planning. 
...
% This principle is a recurring theme, from classic theories of cognitive dissonance to modern frameworks like predictive coding, which treats prediction errors as signals that drive learning via local adjustments.
% A common theme  merges: cognitive work and adaptation are driven by a process of local inconsistency resolution.
% We formalize this process in a graphical model.
% 
...
\commentout{
   %oli1: I think we can do better by slowing down and motivating using my material from below first
   ...
   \todo{cite}.}
% This paper introduces a computational framework formalizing the general principle of Local Inconsistency Resolution (LIR) in graphical models. 

% In practice, this process is constrained by the limits of cognition.
...
First, inconsistency can be difficult to detect, and 
   % it cannot be constructively resolved without awareness. 
...
% This raises a profound question: how should you chose what to focus on?



% Nevertheless, it is a useful 
% It also poses a major question: how exactly should you chose what to focus on?

% We introduce a computational framework that formalizes this process in probabilistic models, 
   % which we call Local Inconsistency Resolution (LIR).
% While this local approach has complexities---fixing one inconsistency can create others out of view---w
% We will show that LIR provides a powerful recipe for learning and (approximate) inference and demonstrate that many foundational techniques in the literature arise naturally as instances of it.
%
% We formalize the process in the language of probability and convex optimization,
% and show how that many foundational techniques in the literature arise naturally as instances of it.
...
% \unskip\footnote{See \eqref{pdg:classfication}}
...
%
   ...
   .%
   \onlyfirsttime{\footnote{The result can also be readily adapted to an entire dataset by replacing $x$ with a high confidence empirical distribution, or mini-batched as in \cref{sec:SGD}.}}
   % $ \theta^{(t+1)}
\end{linked}
...
% analogous choices applied to the analysis of \citet{one-true-loss}
...
%  denote the transformed representations of these tokens. 
...
\begin{prop}
   LIR($\dg M$) in which attention is set to $\varphi_{ij} = \exp \langle k_i, q_j \rangle$, when controlling $x'$, leads to inference in the transformer layer, i.e., 
   % $x_j' = \sum_i \alpha_{i|j} V x_i$ where $\alpha_{i|j} = \mathrm{Softmax}(x_j KQ^{\sf T} x_i / \sqrt{d})$.
   $x_j' = \sum_i
   %  \Big( \exp(x_j KQ^{\sf T} x_i) / \sum_{i'} \exp(x_j KQ^{\sf T} x_{i'}) \Big)
      \alpha_{i|j}
     v_i$.
\end{prop}

Notably, this model forces softmax normalization to be row-wise. 
Controlling the parameters $K,Q,V$ trains the layer, in a sense, but we still do not know whether or not this is equivalent to the standard way of training. 


\subsection{Generative Flow Networks}

\todo{this is the last subsection that needs a major rework; I've removed it for now.}
...
% (albeit in a brittle way, and only by articulating some questionable probabilistic beliefs),
...
\todo{Let's start writing about experiments!}
\section{Conclusion}
